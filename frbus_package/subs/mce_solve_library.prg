' Changes (7/29/14)
'
' 1.  To fix problems with the code for setting terminal conditions,
'     which as not working correctly, changes were made to the
'     subroutine mcz_terminal.  In addition, the call to subroutine
'     mcz_sim when a "single" simulation is being run was changed to
'           call mcz_sim(s_opts)
'     rather than the incorect
'           call mcz_sim(_$_opts)
'
' Changes (6/14)
'
' 1.  Added a new sim type (opt_nonquad) to permit the use of a
'     non-quadratic loss function when simulating optimal policy
'     under committment.  Much of the new code is in a new 
'     subroutine named mcz_opt_nonquad.  A variety of small changes
'     have been made to other subroutines.
'
'     To use the code, set %simtype = "opt_nonquad".  The form of the
'     loss function must be given in a set of equations that are
'     appended to the backward-looking model.  Three equations need
'     to be appended for each target variable:  One for the loss 
'     associated with a target; one for the first derivative of the
'     loss; and one for the second derivative.  The dependent 
'     variables of the appended equations must be named 
'     <target variable>_loss, <target variable>_d1, and
'     <target variable>_d2. 
...
' determine simulation type 
  ...
  if @len(%temp) > 0 then
    ...
    if %simtype = "single" then
      ...
      if %simtype = "opt" or %simtype = "opttc" or %simtype = "opt_nonquad" then
...
' %mcevars_f and %mcevars_b are strings with the names of the MCE
' expectations variables are the appear in the forward-looking
' and backward-looking models.  Terminal values of the variables
' in %mcevars_f are based on the final solution values of the
' variables in %mcevars_b

  %mcevars_f = " "
  %mcevars_b = " "

  %mce_errs = @lower(_$_mce_errs.@members)
  for !ijj = 1 to @wcount(%mce_errs)
    %tmp = @word(%mce_errs,!ijj)
    %tmpz = @mid(%tmp,2)
    %mcevars_b = %mcevars_b + " " + %tmpz
    %tmpw = "w" + @mid(%tmpz,2)
    %mcevars_f = %mcevars_f + " " + %tmpw
    next
...
' code specific to the case of non-quadratic loss
  if %simtype = "opt_nonquad" then
    %opt_loss_names = @wcross(%opt_targ_names,"_loss")
    %opt_loss_d1 = @wcross(%opt_targ_names,"_d1")
    %opt_loss_d2 = @wcross(%opt_targ_names,"_d2")
    endif
...
' text file
  ...
  if %simtype = "opt" or %simtype = "opt_nonquad" then
  ...
  if %simtype = "opt_nonquad" then
    mce_opt_text.append non-quadratic loss function
    endif
  if %simtype = "opttc" then
...
' call appropriate optimization subroutine
  ...
  if %simtype = "opt_nonquad" then
    if %cnstrflag = "yes" then
      @uiprompt("Error:  constraints current cannot be imposed with the opt_nonquad sim type")
      stop
      endif
    call mcz_opt_nonquad
    endif
...
'**************************************************************************
'**************************************************************************
subroutine mcz_opt_nonquad

' Main subroutine for unconstrained optimal control simulations with
' nonquadratic preferences
...
' iterate to minimize loss
  ...
  ' *******************************
  ' compute instrument derivatives, hessian, gradient, direction, and
  ' predicted loss assuming the model is linear and objective function
  ' is quadratic
    if (%ideriv <> "no" and (!opttry = 1 or @abs(!optnonlin) > .1)) then
      call mcz_opt_deriv
      endif
    smpl %evlstart %evlend
    matrix _$_opt_grad = (_$_opt_der_mat)*@vec(@convert(_$_opt_loss_d1))
           
    matrix _$_opt_hess = _$_opt_der_mat _
           * @makediagonal(@vec(@convert(_$_opt_loss_d2))) _
           * @transpose(_$_opt_der_mat)
    if @issingular(_$_opt_hess) = 1 then
      %errstring = "Hessian is singular at iteration " + @str(!opttry)
      @uiprompt(%errstring)
      stop
    ...
    !optloss_p = !optloss + @transpose(@vec(@convert(_$_opt_loss_d1))) _
               * @transpose(_$_opt_der_mat)*(-_$_opt_direction) _
               + .5*@transpose(-_$_opt_direction)*_$_opt_der_mat _
               * @makediagonal(@vec(@convert(_$_opt_loss_d2))) _
               * @transpose(_$_opt_der_mat)*(-_$_opt_direction)
  ...
  ' *******************************
  ' Test for convergence
  ' percentage change in loss from previous iteration has
  ' to be less than !optconv
    !optpchloss =  1-(_$_opt_loss_vec(!opttry+1)/_$_opt_loss_vec(!opttry))
    mce_opt_stats(!opttry+3,4) =  !optpchloss
    if @abs(!optpchloss) < !optconv then
      %opt_converge = "yes"
      mce_opt_text.append At iteration {!opttry}, convergence
      endif
...
' optimal direction and choice of step size, and then solves the model
  ...
  if !opttry = 0 then
    {%_$_mod_b}.makegroup _$_opt_targs_sols {%opt_targ_names}
    if %simtype = "opt_nonquad" then
      {%_$_mod_b}.makegroup _$_opt_loss_sols {%opt_loss_names}
      {%_$_mod_b}.makegroup _$_opt_loss_d1 {%opt_loss_d1}
      {%_$_mod_b}.makegroup _$_opt_loss_d2 {%opt_loss_d2}
      endif
    endif
  ...
  matrix _$_opt_gap_vec = _$_opt_targ_vec - _$_opt_des_vec
  if %simtype = "opt_nonquad" then
    !optloss = @sum(@vec(@convert(_$_opt_loss_sols)))
    else
    !optloss = @sum(@transpose(_$_opt_gap_vec)*_$_opt_wt_mat*_$_opt_gap_vec)
    endif
...
'     
  ...
  ' All but a few backwards looking equations, all of the %mcevars
  if !allbut = 1 then
      %tmp = "-allbut " + %allbut
      read_xml_model( path= %path_b, vars = %tmp, _
                   mod_f=%mod_f, mce_vars= %mce_vars )
  endif

  ' Only a few backwards looking equations, all of the %mcevars
  if !only = 1 then
      read_xml_model( path= %path_b,  vars = %only, _
                   mod_f=%mod_f, mce_vars= %mce_vars )
  endif

  ' All of the backwards looking equations, all of the %mcevars
  if !allbut = 0 and !only = 0 then
      read_xml_model( path= %path_b, _
                  mod_f=%mod_f, mce_vars= %mce_vars )
  endif
...
subroutine mce_append_ezvars(string %model, string %zvars)

  'This should almost never be used

  !nmcevars = @wcount(%zvars)

  for !i = 1 to !nmcevars
    %tmpz = @word(%zvars,!i)
    %tmpw = "w"  + @mid(%tmpz,2)
    %tmpe = "ez" + @mid(%tmpz,2)
    if %tmpz <> "zyh" and %tmpz <> "zyhp" and %tmpz <> "zyht" then
      %tmp1 = %tmpe + "=" + %tmpz + "-" + %tmpw
      else
      %tmp1 = %tmpe + "= log(" + %tmpz + "/" + %tmpw + ")"
      endif
    {%model}.append {%tmp1}
    next
...
subroutine mce_create_auxillary(string %zvars)

   smpl @all
   for !i = 1 to @wcount(%zvars)
     %ztmp = @word(%zvars,!i)
     %wtmp = "w" + @mid(%ztmp,2)
...
subroutine mce_get_zvar_string(string pf_model, string %list)

   string model_temp_name = pf_model
   %w_vars = {model_temp_name}.@endoglist

   %list = " "
   string aaaa = " "
   string aa2 = %w_vars
   smpl @all
   for !i = 1 to @wcount(%w_vars)
     %wtmp = @lower(@word(%w_vars,!i))
     if @left(%wtmp,1) = "w" then
       aaaa = aaaa + %wtmp + " "
       %ztmp = "z" + @mid(%wtmp,2)
       %list = %list + %ztmp + " "
     endif
